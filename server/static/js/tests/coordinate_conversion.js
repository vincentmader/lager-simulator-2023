...
export const test_coordinate_conversions = (io) => {
    ...
    test_conversion_between_world_and_game_display_coords(coordinate_transformer, zoom_level);
    test_conversion_between_cartesian_and_isometric_coords(coordinate_transformer, zoom_level);
};

const test_conversion_between_world_and_game_display_coords = (coordinate_transformer, zoom_level) => {
    let x = 100,
        y = 200;
    let a = new Position(x, y);
    let b = coordinate_transformer.world_to_game_display(a, zoom_level);
    let c = coordinate_transformer.game_display_to_world(b, zoom_level);
    let is_working = (a.x == c.x && a.y == c.y);
    console.log("Conversion \"world -> game_display -> world\" is working: ", is_working);
    if (!is_working) {
        console.log(a, c);
    }
};
